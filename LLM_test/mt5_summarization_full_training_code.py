...
seed = 5
torch.manual_seed(seed)  # 设置 PyTorch 的随机种子，使得每次运行时得到的随机结果是相同的。
torch.cuda.manual_seed(seed)  # 设置 CUDA 的随机种子，用于 GPU 加速时保持随机结果的一致性。
torch.cuda.manual_seed_all(seed)  # 设置所有可用的 CUDA 设备的随机种子，以确保多个设备上的随机结果也是可重现的。
random.seed(seed)  # 设置 Python 内置的随机种子，使得 Python 的随机函数产生的结果是可重复的。
np.random.seed(seed)  # 设置 NumPy 的随机种子，使得 NumPy 的随机函数产生的结果是可重复的。
os.environ['PYTHONHASHSEED'] = str(seed)  # 设置 Python 的哈希种子，用于保持哈希函数的稳定性，也是为了实现可重复性。
...
def test_loop(dataloader, model, mode='Test'):
    ...
    print(
        f"{mode} Rouge1: {result['rouge-1']:>0.2f} Rouge2: {result['rouge-2']:>0.2f} RougeL: {result['rouge-l']:>0.2f}\n")
    return result
...
'''
随着训练的进行，模型在验证集上 ROUGE 值稳步提升。因此 3 轮 Epoch 结束后，会在目录下保存 3 个模型权重：
